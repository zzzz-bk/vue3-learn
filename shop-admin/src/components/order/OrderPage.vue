      ...
      >
        ...
        <el-table-column label="分销信息" width="100">
          <template #default="scoped">
            <el-tag :type="scoped.row.role ? '' : 'info'">{{
              scoped.row.role ? "经理" : "分销员"
            }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="状态" width="100">
          <template #default="scoped">
            <el-tag :type="scoped.row.state ? 'success' : 'danger'">{{
              scoped.row.state ? "已完成" : "未完成"
            }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="200">
          <template #default="scoped">
            <el-button size="small" type="danger" @click="deleteItem(scoped.$index)"
              >删除</el-button
            >
            <el-button size="small" type="primary" @click="callUser(scoped.row)"
              >联系客户</el-button
            >
          </template>
        </el-table-column>
...
function refresh(rt=route){
    orderList.value = Mock.getOrder(rt.params.type);
}
...
function clear() {
  Object.keys(queryParam).forEach(fld=>{queryParam[fld]=''})
  refresh()
}
function exportData(){
...
function dispatchGoods(){
    ElMessage({
        type:'success',
        message:'发送货物:'+JSON.stringify(multipleSelection.value)
    })
}
